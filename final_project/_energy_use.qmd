...
Once I had developed algorithms for calculating a given number of solar panels and turbines given limits on percentage of land to use and the number of states to place renewable technologies in, I could move forward to develop a theoretically optimal system. Here, I focused on minimizing the number of devices that would be needed for a transition to 100% clean, renewable energy. In **section**, I showed that it would be nessecary to provide **x** MWh of renewable energy, in adition to the **x** MWh of hydropower that already exists on the grid. This would 
